...
def decode_predictions(predictions: torch.Tensor, conf_threshold: float = 0.5):
    """
    ...
    pred_boxes = predictions[..., :B*5].view(batch_size, S, S, B, 5)
    ...
    for i in range(S):
        for j in range(S):
            for b in range(B):
                box = pred_boxes[0, i, j, b]
                x, y, w, h, conf = box.tolist()
                if conf >= conf_threshold:
                    class_idx = torch.argmax(pred_class[0, i, j]).item()
